...
const ManagerCard: FC<P> = ({ index, manager, getSingleManager, entity }) => {
    return <div key={index} className='w-full border rounded-md  flex flex-col p-2 gap-4 shadow-md'>
        <div className='flex justify-between gap-3 w-full '>
            <img src={import.meta.env.VITE_MANAGER_IMAGE} className='w-1/3 rounded-md shadow-md' alt='sarukh' />
        ...
        <p className='flex items-center gap-3 p-2'><span>{entity?.name}</span><span className='text-gray-400 '>[Branch Name]</span></p>
    </div>
}
