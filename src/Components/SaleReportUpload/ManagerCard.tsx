...
const ManagerCard:FC<P> =({ key })=>{
  return <div key={key} className='w-full border rounded-md  flex flex-col p-2 gap-4 shadow-md'>
  <div className='flex justify-between gap-3 w-full '>
      ...
      <div className='w-2/3 self-start gap-3 flex items-center justify-end'>
          <div className='flex flex-col'>
              <h1 className='border-b-2 border-gray-500 pl-3 pb-1'>Pratiksha Ekbote</h1>
              <p className='self-end text-gray-500 '>MANAGER</p>
          </div>
          <Button variant='contained' startIcon={<MdOutlineModeEditOutline size={25} />} children="EDIT" />
      </div>
  </div>
